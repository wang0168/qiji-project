package tts.project.qiji.engineer.fragment.orders;
 ...
 * 待确认
 */
public class EngWaitConfirmFragment extends BaseFragment {
    ...
    private void initData(String response) {
        ...
        adapter.setOnItemClickListener(new OrderEnginerrAdapter.OnItemClickListener() {
            ...
            public void SerivceCcom(int position) {
                startActivityForResult(new Intent(getActivity(), EngComServerActivity.class).putExtra("order_id", datas.get(position).getOrder_id()), 1000);
            }
