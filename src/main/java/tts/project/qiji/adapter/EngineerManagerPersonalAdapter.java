 ...
 */
public class EngineerManagerPersonalAdapter extends TTSBaseAdapterRecyclerView<UserManagerDataBean.SignBean> {
    private Context mContext;
    private List<UserManagerDataBean.SignBean> mData;

    public EngineerManagerPersonalAdapter(Context context, List<UserManagerDataBean.SignBean> data) {
        super(context, data);
        mData = data;
        mContext = context;
    }
    ...
    public void onBindViewHolder(TTSBaseAdapterRecyclerView.ViewHolder holder, final int position) {
        ...
        if (!TextUtils.isEmpty(mData.get(position).getQy_name())) {
            ViewHolder.signed_company.setText(mData.get(position).getQy_name());
        } else {
