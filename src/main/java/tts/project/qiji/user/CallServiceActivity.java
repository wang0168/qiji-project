...
//                    CustomUtils.showTipShort(CallServiceActivity.this,"已经是最后一级分类啦");
                    startActivityForResult(new Intent(CallServiceActivity.this, OrderActivity.class).
                            putExtra("price", data.getSort_types().get(position).getPrice()).putExtra("service_id", data.getSort_types().get(position).getFuwu_id())
                            .putExtra("service_type", data.getSort_types().get(position).getName()), 10001);
                    return;
                }
